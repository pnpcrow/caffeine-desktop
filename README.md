...
| 마우스 커서 찾기 (0.3.0) | `rust/src/cursor_find.rs` — 마우스를 좌우로 빠르게 흔들면(550ms 내 방향전환 3회+220px) 커서를 흔든 강도만큼 확대하고 원형 파형을 퍼뜨려 위치를 알려 줌. 멀티 모니터에서는 커서가 없는 모니터 중앙에 커서 방향 8방향 화살표 표시(설정으로 끄기 가능). 이펙트 동안 OSD에 포인터 아이콘 점등. 가림 중에는 흔들기가 해제 제스처로 동작하므로 상호배제 |
