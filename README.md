...
| 마우스 커서 찾기 (0.3.0) | `rust/src/cursor_find.rs` — 마우스를 좌우로 빠르게 흔들면(550ms 내 방향전환 3회+220px) 선택한 이펙트로 커서 위치를 알려 줌: 커서 확대(흔든 강도에 비례, 별도 토글), 원형 파형(별도 토글), 커서가 없는 모니터 중앙의 모니터 높이 비례 큰 화살표(8방향, 별도 토글). 기능이 켜져 있으면 OSD에 포인터 아이콘 상시 표시(커피잔·눈 아이콘과 같은 시맨틱). 가림 중에는 흔들기가 해제 제스처로 동작하므로 상호배제 |
